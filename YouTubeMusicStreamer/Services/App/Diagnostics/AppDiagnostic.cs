// This file is part of YouTubeMusicStreamer.
// Copyright (C) 2026 Dominic Ris
// 
// YouTubeMusicStreamer is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published
// by the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
// 
// YouTubeMusicStreamer is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Affero General Public License for more details.
// 
// For full license text, see the LICENSE file in the project’s root directory.
// 
// You should have received a copy of the GNU Affero General Public License
// along with YouTubeMusicStreamer. If not, see <https://www.gnu.org/licenses/>.

namespace YouTubeMusicStreamer.Services.App.Diagnostics;

public enum AppDiagnosticSubsystem
{
    Startup,
    Settings,
    Twitch,
    YouTube,
    Commands,
    Updates,
    Logging
}

public enum AppDiagnosticSeverity
{
    Info,
    Warning,
    Error
}

public enum AppDiagnosticCategory
{
    Auth,
    Configuration,
    Connectivity,
    Persistence,
    Startup,
    Update,
    InternalFault
}

public enum AppDiagnosticVisibility
{
    DiagnosticsOnly,
    StatusOnly,
    Toast
}

public sealed record AppDiagnostic(
    Guid Id,
    AppDiagnosticSubsystem Subsystem,
    AppDiagnosticSeverity Severity,
    AppDiagnosticCategory Category,
    string Summary,
    string? Detail,
    Exception? Exception,
    string? ExceptionText,
    DateTimeOffset CreatedAtUtc,
    AppDiagnosticVisibility Visibility)
{
    public string? ExceptionDisplayText => Exception?.ToString() ?? ExceptionText;
}
