...
public class StartCommand(CommandBag bag) : CommandBase(bag)
{
    [UsedImplicitly]
    protected async Task<CommandExecutionResult<StartResult>> ExecuteCommandLogicAsync(
    ...
    )
    {
        ...
        {
            return new CommandExecutionResult<StartResult>(false);
        }
        ...
        await Bag.YouTubeService.RestClient.ChangeVideo(videoId);

        return new CommandExecutionResult<StartResult>(true, new StartResult
        {
