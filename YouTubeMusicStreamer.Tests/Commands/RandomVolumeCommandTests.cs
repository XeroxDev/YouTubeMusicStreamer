...
public class RandomVolumeCommandTests
{
    [Fact]
    public async Task ExecuteCommandLogicAsync_ReturnsRandomVolume_WhenWorkflowSucceeds()
    {
        var workflow = new FakeYouTubeCommandWorkflow
        {
            RandomVolumeResult = new YouTubeWorkflowResult<int>(YouTubeWorkflowStatus.Success, 42)
        };
        var command = new RandomVolumeCommand(workflow);

        var result = await CommandTestSupport.InvokeAsync(
            command,
            CommandTestSupport.CreateContext("RandomVolumeCommand", "randomvolume", "!randomvolume"));

        Assert.Equal(1, workflow.SetRandomVolumeCallCount);
        var typed = Assert.IsType<CommandResult<RandomVolumeCommand.RandomVolumeResult>>(result);
        Assert.Equal(CommandExecutionStatus.Fulfilled, typed.Status);
        Assert.Equal(42, typed.Value!.Volume);
    }
    ...
    public async Task ExecuteCommandLogicAsync_ReturnsBlockedMissingIntegration_WhenWorkflowFails()
    {
        var workflow = new FakeYouTubeCommandWorkflow
        {
            RandomVolumeResult = new YouTubeWorkflowResult<int>(YouTubeWorkflowStatus.Blocked, Message: "YTMDesktop missing.")
        };
        var command = new RandomVolumeCommand(workflow);
